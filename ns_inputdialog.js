 ...
 *
 * v3.0.4  2025-09-21
 * Changed default button value from 'true' to '1'.
 * Added css colors for button with value 1
 *
 */
		...
		function InputDialog(options) {
				...
				for (let i = 0; i < _options.fields.length; i++) {
					...
					if (_options.fields[i].actionButtons.length === 0 || _options.fields[i].actionButtons.indexOf(buttonId) >= 0) {
						...
						if (_options.fields[i].isMandatory && !text) {
							...
							window.nsInputFieldValues.push(text);
						}
					}
				}
			...
			function _buildBody() {
				...
				for (let i = 0; i < _options.fields.length; i++) {
					...
					if (field.caption) {
						output += '<span class="uir-label" data-nsps-type="field_label">'+ field.caption.toString().toUpperCase() + '</span><br/>';
					}
		...
		function create(options, success, failure) {
			doDialog(new InputDialog(options).build(), 'dialog')
				.then(function(result) {
					if (success) {
						success(result, window.nsInputFieldValues)
					}
				})
