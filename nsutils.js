...
define(['N/record', 'N/runtime', 'N/query'],
	...
	function(record, runtime, query) {
		 ...
		 * @param {nlobj} searchObj netsuite saved search object
		 * @return Search Results
		 */
		function getAllSearchResults(searchObj) {
			const set = searchObj.run();
			let holder = [];
			let i = 0;
			while (true) {
				const result = set.getRange({
					start: i,
					end: i + 1000
				});
				if (!result) break;
		...
		/**
		 * Retrieves data from paged search objects
		 * @param {object} params
		 * @param {query.PagedData} params.pagedData
		 * @param {number} params.startIndex slice
		 * @param {number} params.endIndex slice
		 * @param {function} params.callback
		 * @return {[]}
		 */
		const getAllPagedResults = (params) => {
			let holder = [];
			const { pageSize } = params.pagedData;
			let startPage = isEmpty(params.startIndex) ? 0 : Math.round(params.startIndex / pageSize) - 1;
			if (startPage < 0) {
				startPage = 0;
			}
			let endPage = isEmpty(params.endIndex)
				? params.pagedData.pageRanges.length - 1
				: Math.round(params.endIndex / pageSize);
			if (endPage >= params.pagedData.pageRanges.length) {
				endPage = params.pagedData.pageRanges.length - 1;
			}

			let i = startPage;
			while (i <= endPage && params.pagedData.count > 0) {
				holder = [
					...holder,
					...(typeof params.callback == 'function'
						? params.callback(params.pagedData.fetch(i).data.asMappedResults()) || []
						: params.pagedData.fetch(i).data.asMappedResults())
				];
				i++;
			}

			if (!isEmpty(params.startIndex) && !isEmpty(params.endIndex)) {
				holder = holder.slice(params.startIndex, params.endIndex);
			}
			if (isEmpty(params.startIndex) && !isEmpty(params.endIndex)) {
				holder = holder.slice(0, params.endIndex);
			}
			if (!isEmpty(params.startIndex) && isEmpty(params.endIndex)) {
				holder = holder.slice(params.startIndex);
			}
			return holder;
		};
		...
		return {
			getAllSearchResults,
			getAllPagedResults,
			isTransactionStandAlone,
			isOWA,
			getSubsidiaryTimeZoneAndDateFormat,
			getUserTimeZoneAndDateFormat,
